...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  final LoginPatientUseCaseContract _loginPatientUseCase =
      LoginPatientUseCaseContract.get();
  ...
  void _fetchBasicLoginDataEventToState(
      LoginEvent event, Emitter<LoginState> emit) async {
    emit(LoginStateLoading());

    _firebaseToken = await LocalStorageProvider.readData(
        RepositoryPathConstant.firebaseToken.path); //
  ...
  void _navigateToEventToState(
      LoginEventNavigateTo event, Emitter<LoginState> emit) {
    _dispose();
  ...
  void _loginPatientEventToState(
      LoginEventLoginPatient event, Emitter<LoginState> emit) async {
    emit(LoginStateLoading());
    ...
    final response =
        await _loginPatientUseCase.run(event.signInPatientDomainModel);
